...
//         string retour ( "" );
    ...
    {
	    vector<string> retour=vec1;
	    for ( int i = 0; i < ( int ) vec2.size(); i++ )
	    {
		retour.push_back ( vec2.at ( i ) );
	    }
	    return retour;
...
//         str_mod = regex_replace ( str_mod, rex, replace );
    ...
    {
        param to_return;
        to_return.outputAlignments = p.outputAlignments;
        ...
        to_return.alignmentFile = p.alignmentFile;
        return to_return;
    }
    string printParams ( param p )
    {
        ...
        s << "outputAlignments = " << p.outputAlignments << endl;
        s << "hypothesisFile = " << p.hypothesisFile << endl;
        s << "referenceFile = " << p.referenceFile << endl;
        s << "alignmentFile = " << p.alignmentFile << endl;
	...
	in.push(file);
	boost::iostreams::copy(in, to_return);
    }
